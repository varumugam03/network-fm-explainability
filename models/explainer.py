...
class NetworkExplainer(nn.Module):
    ...
    def __init__(self, mlp_checkpoint: str, llm_model_id: str, mlp_config: dict, device: str = "cuda"):
        super().__init__()
        self.device = device
        ...
        base_llm = AutoModelForCausalLM.from_pretrained(llm_model_id, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2")
    ...
    def forward(self, mlp_features, text_prompts, labels=None):
        ...
        for i in range(batch_size):
            ...
            label_ids = torch.cat([torch.full((ignore_len,), -100, device=self.device, dtype=torch.long), target_tokens.squeeze(0)])
        ...
        # 4. LLM Forward
        outputs = self.llm(inputs_embeds=padded_embeds, attention_mask=padded_mask, labels=padded_labels)
